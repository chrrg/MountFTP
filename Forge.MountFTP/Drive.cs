...
namespace Forge.MountFTP
{
    ...
    public class Drive
    {
        ...
        public string Mount()
        {
            ...
            //FtpClient.LogToConsole = true;
            //FtpClient.OnLogEvent += new LogEventHandler(OnFTPSClientLogCommand);
            FtpClient.OnLogEvent += (FluentFTP.FtpTraceLevel level, string str) =>
            {
                System.Console.WriteLine("FTP:"+level+":"+str);
                return;
            };
            var dokanFtpClient = new DokanFtpClient(FtpClient, options);
        ...
        void OnFTPSClientLogCommand(object sender, LogEventArgs args)
        {
            if (FtpCommand != null)
            {
                FtpCommand(this, args);
            }
        }
